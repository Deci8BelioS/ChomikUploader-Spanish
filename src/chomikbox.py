...
def unescape(string):
    ...
    result = _hex.sub(
        ...
        _dec.sub(lambda x: unichr(int(x.group(1))), _char.sub(_char_unescape, string)),
    )
...
class Chomik(object):
    ...
    def send(self, content):
        ...
        while counter < 100:
            try:
                ...
                self.view.print_("Error al conectar: " + str(error))
                self.view.print_("Prueba " + str(counter) + " de 100")
                counter += 1
    ...
    def relogin(self):
        ...
        xml_dict = [
            (
                ...
                [
                    ...
                    ("passHash", password),
                    ("ver", "4"),
                    ("client", [("name", "chomikbox"), ("version", version)]),
                ],
            )
        ]
        ...
        status = resp_dict["s:Envelope"]["s:Body"]["AuthResponse"]["AuthResult"][
            "a:status"
        ]
        if status != "Ok":
            self.view.print_("Error(volviendo a iniciar sesion):")
            self.view.print_(status)
            return False
        try:
            chomik_id = resp_dict["s:Envelope"]["s:Body"]["AuthResponse"]["AuthResult"][
                "a:hamsterId"
            ]
            ses_id = resp_dict["s:Envelope"]["s:Body"]["AuthResponse"]["AuthResult"][
                "a:token"
            ]
            self.ses_id = ses_id
            self.chomik_id = chomik_id
    ...
    def get_dir_list(self, folder_id=0, folder_dom_root={}):
        ...
        resp = self.send(header)
        resp_dict = self.soap.soap_xml_to_dict(resp)
        status = resp_dict["s:Envelope"]["s:Body"]["FoldersResponse"]["FoldersResult"][
            "a:status"
        ]
        if status != "Ok":
            self.view.print_("Error(descargando la lista de carpetas):")
            self.view.print_(status)
            return False
        if folder_id == 0:
            self.folders_dom = resp_dict["s:Envelope"]["s:Body"]["FoldersResponse"][
                "FoldersResult"
            ]["a:folder"]
        else:
            # print "Get list dir:",  resp_dict['s:Envelope']['s:Body']['FoldersResponse']['FoldersResult']['a:folder']
            # FIXME danger
            folder_dom_root["folders"] = resp_dict["s:Envelope"]["s:Body"][
                "FoldersResponse"
            ]["FoldersResult"]["a:folder"]["folders"]
            return True
        return True
    ...
    def chdirs(self, directories):
        ...
        folders = self.cur_fold + [
            i.replace("/", "") for i in directories.split("/") if i != ""
        ]
        fold = []
        for f in folders:
            if f == "..":
    ...
    def mkdir(self, dirname, folder_id=None):
        ...
        self.view.print_("Creando", dirname, "directorio")
        ...
        xml_dict = [
            (
                "ROOT",
                [("token", self.ses_id), ("newFolderId", folder_id), ("name", dirname)],
            )
        ]
        ...
        resp = self.send(header)
        resp_dict = self.soap.soap_xml_to_dict(resp)
        status = resp_dict["s:Envelope"]["s:Body"]["AddFolderResponse"][
            "AddFolderResult"
        ]["status"]["#text"]
        if status == "Ok":
            self.view.print_("Exito de creación\r\n")
            return True
        else:
            error_msg = resp_dict["s:Envelope"]["s:Body"]["AddFolderResponse"][
                "AddFolderResult"
            ]["errorMessage"]["#text"]
            if error_msg == "NameExistsAtDestination":
                return True
            else:
                self.view.print_("Creacion fallida")
    ...
    def rmdir(self):
        ...
        """
        self.relogin()
        self.view.print_("Eliminando el directorio actual")
        ########################
        xml_dict = [
            (
                "ROOT",
                [("token", self.ses_id), ("folderId", self.folder_id), ("force", "1")],
            )
        ]
        ...
        header = """POST /services/ChomikBoxService.svc HTTP/1.1\r\n"""
        header += (
            """SOAPAction: http://chomikuj.pl/IChomikBoxService/RemoveFolder\r\n"""
        )
        header += """Content-Type: text/xml;charset=utf-8\r\n"""
        if self.cookie != "":
            header += """Cookie: __cfduid={0}\r\n""".format(self.cookie)
        ...
        resp = self.send(header)
        resp_dict = self.soap.soap_xml_to_dict(resp)
        status = resp_dict["s:Envelope"]["s:Body"]["RemoveFolderResponse"][
            "RemoveFolderResult"
        ]["a:status"]
        if status == "Ok":
            self.view.print_("Eliminacion exitosa\r\n")
            return True
        else:
            self.view.print_("Error de eliminacion")
            self.view.print_(status)
            return False
    ...
    def upload(self, filepath, filename):
        ...
        if token == None:
            ...
            result = self.__upload_with_resume_option(
                filepath,
                filename,
                token,
                stamp,
    ...
    def __upload_with_resume_option(
        self, filepath, filename, token, stamp, server, port, chomik_id, folder_id
    ):
        try:
            result = self.__upload(filepath, filename, token, stamp, server, port)
        except (socket.error, socket.timeout) as e:
            self.view.print_("Reanudar\n")
            result = self.resume(
                filepath, filename, folder_id, chomik_id, token, server, port, stamp
            )
        return result
    ...
    def __upload_get_tokens(self, filepath, filename):
        ...
        resp = self.send(header)
        resp_dict = self.soap.soap_xml_to_dict(resp)
        status = resp_dict["s:Envelope"]["s:Body"]["UploadTokenResponse"][
            "UploadTokenResult"
        ]["a:status"]
        if status != "Ok":
            self.view.print_("Error(descargando informacion de hamster):")
            self.view.print_(status)
            return None, None, None, None
        try:
            self.token = resp_dict["s:Envelope"]["s:Body"]["UploadTokenResponse"][
                "UploadTokenResult"
            ]["a:key"]
            self.stamp = resp_dict["s:Envelope"]["s:Body"]["UploadTokenResponse"][
                "UploadTokenResult"
            ]["a:stamp"]
            self.server = resp_dict["s:Envelope"]["s:Body"]["UploadTokenResponse"][
                "UploadTokenResult"
            ]["a:server"]
            self.locale = resp_dict["s:Envelope"]["s:Body"]["UploadTokenResponse"][
                "UploadTokenResult"
            ]["a:locale"]
            self.server, _, self.port = self.server.partition(":")
            return self.token, self.stamp, self.server, self.port
        except IndexError as e:
            self.view.print_("Error(descargando informacion de hamster):", e)
            self.view.print_(resp)
            return None, None, None, None
    ...
    def __upload(self, filepath, filename, token, stamp, server, port):
        ...
        header, contenttail = self.__create_header(
            server, port, token, stamp, filename, size
        )
        ...
        if '<resp res="1" fileid=' in resp:
            ...
            try:
                ...
                self.view.print_("Error(envio fallido):\r\n", error_msg)
            except IndexError:
                pass
            self.view.print_(resp)
            return False
    ...
    def __create_header(
        self, server, port, token, stamp, filename, size, resume_from=0
    ):
    ...
    #####################################################
    def resume(
        self, filepath, filename, folder_id, chomik_id, token, server, port, stamp
    ):
        self.relogin()
        self.chomik_id = chomik_id
        self.folder_id = folder_id
        filename_tmp = change_coding(filename)
        filesize_sent = self.__resume_get_tokens(
            filepath, filename, token, server, port
        )
        if (filesize_sent == -1) or token == None:
            if self.debug:
                self.view.print_("Reanudar ", filename_tmp)
                self.view.print_("Tamano de archivo enviado", filesize_sent)
            return False
        else:
            return self.__resume_with_resume_option(
                filepath,
                filename,
                token,
                server,
    ...
    def __resume_with_resume_option(
        self,
        filepath,
        filename,
        token,
        ...
        try:
            result = self.__resume(
                filepath, filename, token, server, port, stamp, filesize_sent
            )
            self.view.print_("Resultado", result)
        except (socket.error, socket.timeout) as e:
            self.view.print_("Reanudar\n")
            result = self.resume(
                filepath, filename, folder_id, chomik_id, token, server, port, stamp
    ...
    def __resume_get_tokens(self, filepath, filename, token, server, port):
        ...
        try:
            filesize_sent = int(
                re.findall(
                    """<resp file_size="([^"]*)" skipThumbnails="[^"]*" res="1"/>""",
                    resp,
                )[0]
            )
            return filesize_sent
        except IndexError as e:
            self.view.print_("No se pudo reanudar la descarga")
            self.view.print_(resp)
            return -1
    ...
    def __resume(self, filepath, filename, token, server, port, stamp, filesize_sent):
        ...
        pb = view.ProgressBar(
            total=size, rate_refresh=0.5, count=filesize_sent, name=filepath
        )
        self.view.add_progress_bar(pb)
        last_time = time.time()
