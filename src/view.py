...
def change_unit_bytes(value):
    if value < 1024:
        ...
        return (value / 1024.0, 'kB')
    elif value < 1024 ** 3:
        return (value / 1048576.0, 'MB')
    else:
...
class ProgressBar(object):
    ...
    def update(self, count):
        ...
        if now - self.last_update > 0.5:  # FIXME
            ...
            hist = [i for i in self.history if i is not None]
    ...
    def get_meter(self, **kw):
        ...
        return '[%s>%s] %d%%  %.1f%s/sec  %.1f%s/%.1f%s  %.1f%s' % (
            bar, pad, perc, rate_current, unit, downloaded, unit_d, total, unit_t, rest_time, unit_time)
